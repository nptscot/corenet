...
df_metrics <- data.frame(
  SpatialCoverage           = spatial_coverage,            # e.g. 0-100
  ZoneConnectivity          = zone_connectivity,           # e.g. 0-100
  CyclingPotentialCoverage  = cycling_potential_coverage,  # e.g. 0-100
  CitywideDensityRatio      = citywide_density_ratio_scaled, 
  PopulationCoverage        = population_coverage,         # e.g. 0-100
  ODCoverageCountBased      = od_coverage_count,           # e.g. 0-100
  ODCoverageDemandWeighted  = od_coverage_demand,          # e.g. 0-100
  Directness                = directness_scaled,           # e.g. 0-100
  GlobalEfficiency          = global_efficiency_scaled,    # e.g. 0-100
  LocalEfficiency           = local_efficiency_scaled      # e.g. 0-100
)

# Optional: If you prefer to keep fewer metrics in your radar chart,
# remove or comment out lines to show just the ones you want.
# You can define uniform boundaries or use observed max/min:
max_vals <- apply(df_metrics, 2, max, na.rm = TRUE)
min_vals <- rep(0, ncol(df_metrics))  # set everything to start at 0

# If you prefer a consistent 100 across all metrics:
# max_vals <- rep(100, ncol(df_metrics))

df_radar <- rbind(max_vals, min_vals, df_metrics)

# fmsb will read the first two rows as the upper and lower range for the axes
# Optional: rename columns for nicer display in legend/axes
colnames(df_radar) <- c(
  "Spatial\nCoverage",
  "Zone\nConnectivity",
  "Cycling\nPotential",
...
radarchart(
  df_radar,
  axistype = 1,    # type of axis (see ?radarchart)
  seg = 5,         # how many segments on each axis
  pcol = rgb(0.2, 0.5, 0.5, 0.9),  # polygon line color
  pfcol = rgb(0.2, 0.5, 0.5, 0.5), # fill color
  plwd = 2,                        # polygon line width
  cglcol = "grey",                 # grid line color
  cglty = 1,                       # grid line type
  axislabcol = "grey",
  caxislabels = seq(0, max(max_vals), length.out = 6),  # axis label breaks
  title = "City Cycling Infrastructure - Radar Chart"
)

# If you have multiple scenarios or multiple rows of data to compare,
# you can add more rows to df_radar after the first two min/max rows.
# For each row beyond the second, radarchart will add another polygon.
