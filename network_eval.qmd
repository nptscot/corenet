...
```{r}
rnet_core_zone_length = list()

for (city_name in city_names[4]) {
  city_boundary = lads |> filter(LAD23NM == city_name)
  rnet_core_zone = sf::st_intersection(rnet_core, city_boundary)
  rnet_core_zone$length_m = as.numeric(st_length(rnet_core_zone))
  total_length_m = sum(rnet_core_zone$length_m, na.rm = TRUE)
  total_length_km = total_length_m / 1000
  rnet_core_zone_length[[city_name]] = total_length_km
}
```
...
```{r}
rnet_cycle_zone_length = list()

for (city_name in city_names[4]) {
  city_boundary = lads |> filter(LAD23NM == city_name)
  intermediate_zone_city = intermediate_zone_city |> filter(LAD23NM == city_name)
  rnet_cycle_zone = sf::st_intersection(rnet_cycle, city_boundary)
  rnet_cycle_zone$length_m = as.numeric(st_length(rnet_cycle_zone))
  total_length_m = sum(rnet_cycle_zone$length_m, na.rm = TRUE)
  total_length_km = total_length_m / 1000
  rnet_cycle_zone_length[[city_name]] = total_length_km
}

mapview(rnet_cycle_zone, color = "red") + mapview(rnet_core_zone, color = "blue") + mapview(intermediate_zone_city, zcol = "Name")
```
